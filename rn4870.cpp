...
boolean RN4870::setName(char *newName, char nameLen){
    ...
    unsigned char ptrData = strlen(SET_NAME);
...
boolean RN4870::setBaudRate(char bufferParam)
{
    ...
    unsigned char ptrData = strlen(SET_BAUDRATE);
...
boolean RN4870::setPowerSave(boolean powerSave){
    ...
    // write CMD in buffer
    unsigned char ptrData = strlen(SET_POWERSAVE);
...
void RN4870::validateAnswer(void) {
    switch(status) {
    case enterConfmode:
